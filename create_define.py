...
def Create_Define(wb,ta_var):
    ...
    # store the grid content of the Domains sheet in a dictionary for easier access when creating the Define.xml
    for i in range(1, sh_D.max_column + 1):
        content[0].append(sh_D.cell(row=1, column=i).value)
        for r in range(2, sh_D.max_row+1):
            m=r-1
            if m not in content:
                content[m]=[]
            content[m].append(sh_D.cell(row=r, column=i).value)
    
    root = ET.Element("ODM", FileType="Snapshot", ODMVersion="1.3.2", CreationDateTime="2024-06-01T12:00:00", xmlns="http://www.cdisc.org/ns/odm/v1.3", xmlns_def="http://www.cdisc.org/ns/def/v2.1")
    study = ET.SubElement(root, "Study", OID="STUDY1")
    metaDataVersion = ET.SubElement(study, "MetaDataVersion", OID="MDV1", Name="Metadata Version 1", Description="Metadata Version 1")
    ET.SubElement(metaDataVersion, "Standards", Name="SDTMIG", Type="IG", Version="3.1.2", CommentOID="COM.STD1")
    for domain in range(1, len(content)):
        if content[domain][0] is not None:
            domainElement = ET.SubElement(metaDataVersion, "ItemGroupDef", OID=content[domain][0], Name=content[domain][1], Domain=content[domain][2], Purpose=content[domain][3],SasDatasetName=content[domain][4], Repeating=content[domain][5], IsReferenceData=content[domain][6], Structure=content[domain][7], ArchiveLocation=content[domain][8], StandardOID=content[domain][9])
